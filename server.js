...
// Add this route for handling file downloads
app.get('/api/doctor/appointments/:appointmentId/files/:filename', authenticateToken, async (req, res) => {
    try {
        ...
        const file = appointment.attachments.find(a => a.filename === filename);
        if (!file) {
            return res.status(404).json({ error: 'File not found' });
        }

        // Send the file
        const filePath = path.join(__dirname, 'public', 'uploads', 'appointments', filename);
